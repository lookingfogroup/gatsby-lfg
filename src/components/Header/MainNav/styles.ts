...
export const MainNavItem = motion.custom(styled(Link)`
  ${tw`relative text-red-900 border-b border-transparent hover:text-red-900 ml-0 sm:ml-8 mt-3 sm:mt-0`};
  width: max-content;
  ...
  &:before {
    ${tw`absolute w-full bg-yellow-400 h-px left-0 invisible`};
    content: '';
    bottom: -1px;
    transform: scaleX(0);
    transition: 0.2s;
...
export const ToogleMainNav = styled.button<StyledProps>`
  ...
  span {
    ${tw`bg-red-500 inline-block w-6 h-px`};
    transition: 0.2s;
    ...
    &:nth-child(2) {
      ${tw`bg-yellow-400 inline-block w-8 h-px`};
